...
sub main_page : StartRunmode {
    ...
    my $clubreg_track_ids = $self->dbh->selectall_hashref( "SELECT clubreg_id, track_id, date, end_date, registration FROM clubreg_urls", "clubreg_id" );
...
sub upsertClubRegistrationEvent {
    ...
    $dbh->do( "UPDATE clubreg_urls SET registration = ? WHERE clubreg_id = ?", {}, $registration, $id ) if $registration;
